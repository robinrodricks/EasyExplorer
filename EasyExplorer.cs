...
namespace EasyExplorer {
	...
	public partial class Explorer : UserControl {
		...
		public Explorer() {
			...
			startupDir = SpecialFolders.Other;
			if (Config_Filtered && Config_OnlyShowPaths != null && Config_OnlyShowPaths.Count > 0) {
				otherStartupDir = Config_OnlyShowPaths[0];
			} else {
				otherStartupDir = Config_InitialPath;
			}
			...
			navigationBar.Visible = Config_ShowAddressBar;

			//shellBrowser = new ShellBrowser(this);

			FileBrowser_HandleCreated(this, null);
		...
		private void InitBaseItems() {
			...
			navAddressBox.Items.Add(new ExplorerComboItem(ShellBrowser.DesktopItem, 0));

			navAddressBox.CurrentItem = (ExplorerComboItem)navAddressBox.Items[0];
			...
			foreach (ShellItem desktopChild in ShellBrowser.DesktopItem.SubFolders) {
				...
				if (Config_Filtered && desktopChildNode.Text != ShellBrowser.MyComputerName) {
					continue;
				}

				navAddressBox.Items.Add(new ExplorerComboItem(desktopChild, 1));
				...
				if (desktopChildNode.Text == ShellBrowser.MyComputerName) {
					...
					foreach (ShellItem myCompChild in desktopChild.SubFolders) {
						...
						string driveLetter = BetweenString(myCompChildNode.Name, "(", ")", 0, false, false).Replace(":", "").ToUpper();

						// check if filtered out
						bool show = true;
						if (Config_Filtered && Config_OnlyShowPaths != null) {
							show = false;
							foreach (string path in Config_OnlyShowPaths) {
						...
						// only add if wanted
						if (show) {
							navAddressBox.Items.Add(new ExplorerComboItem(myCompChild, 2));
		...
		private string BetweenString(string text, string textStart, string textEnd, int startAt = 0, bool returnAll = false, bool forward = true) {
			int start;
			if (forward) {
				start = text.IndexOf(textStart, startAt, StringComparison.Ordinal);
			} else {
				start = text.LastIndexOf(textStart, text.Length - startAt, StringComparison.Ordinal);
				if (textStart == textEnd) {
					start = text.LastIndexOf(textStart, start - 1, StringComparison.Ordinal);
				}
			}
			if (start != -1) {
				start += textStart.Length;
				int end = text.IndexOf(textEnd, start + 1, StringComparison.Ordinal);
				if (end != -1) {
 ...
 break;
		...
		private bool ExtendTreeNode(TreeNode node, bool overwrite, IntPtr handle) {
			if (overwrite || !IsExtended(node)) {
				...
				if (nodeItem.Expand(false, true, handle)) {
					...
					List<TreeNode> newNodesArray = new List<TreeNode>();
					for (int i = 0; i < nodeItem.SubFolders.Count; i++) {
						ShellItem subDir = nodeItem.SubFolders[i];

						// check if filtered out
						bool show = true;
						if (Config_Filtered) {
							show = false;
							if (Config_OnlyShowPaths != null) {
								foreach (string path in Config_OnlyShowPaths) {
									if (path.StartsWith(subDir.Path) || subDir.Path.StartsWith(path)) {
										show = true;
									}
								}
							}
						...
						// add if wanted
						if (show) {
							TreeNode treeNode = new TreeNode(subDir.Text, subDir.ImageIndex, subDir.SelectedImageIndex);
							newNodesArray.Add(treeNode);
							treeNode.Tag = subDir;

							if (subDir.HasSubfolder) {
					...
					node.Nodes.AddRange(newNodesArray.ToArray());
		...
		private bool SetNewPath(ShellItem oldItem, ShellItem newItem) {
			...
			if (oldItem != newItem && newItem.Expand(true, false, Handle)) {
				...
				if (oldItem != null) {
					bool used = false;
					foreach (Explorer br in ShellBrowser.Browsers) {
				...
				if (Config_OnSelectFolder != null && newItem.IsFolder) {
					Config_OnSelectFolder(newItem.Path);
				}
		...
		void onClickFile(object sender, EventArgs e) {
			if (Config_OnSelectFile != null && fileView.SelectedItems != null && fileView.SelectedItems.Count > 0) {
				Config_OnSelectFile((fileView.SelectedItems[0].Tag as ShellItem).Path);
			}
		}
		...
		[Category("Options"),
		 Description("Show or hide the address bar.")]
		public bool ShowAddressBar {
			get { return Config_ShowAddressBar; }
			set {
				Config_ShowAddressBar = value;
				navigationBar.Visible = Config_ShowAddressBar;
			}
		}

		[Category("Options"),
		 Description("Show or hide the 'folders' button.")]
		public bool ShowFoldersButton {
			get { return Config_ShowFoldersButton; }
			set {
				Config_ShowFoldersButton = value;
				navFoldersButton.Visible = Config_ShowFoldersButton;
			}
		}

		[Category("Options"),
		 Description("Show or hide the folders tree on the left side.")]
		public bool ShowFoldersTree {
			get { return Config_ShowFoldersTree; }
			set {
				Config_ShowFoldersTree = value;
				navFoldersButton.Checked = Config_ShowFoldersTree;
			}
		}

		[Category("Options"),
		 Description("Change the width of the folder tree.")]
		public int FolderTreeWidth {
			get { return browseSplitter != null ? browseSplitter.SplitterDistance : Config_FolderTreeWidth; }
			set {
				Config_FolderTreeWidth = value;
				browseSplitter.SplitterDistance = Config_FolderTreeWidth;
			}
		}

		// CONFIG!
		public bool Config_ShowAddressBar = true;
		public bool Config_ShowFoldersButton = true;
		public bool Config_ShowFoldersTree = true;
		public int Config_FolderTreeWidth = 300;
		public bool Config_Filtered = false;
		public List<string> Config_OnlyShowPaths = null;//new List<string> { @"D:\Config\AppSettings" };
		public string Config_InitialPath = null;
		public Action<string> Config_OnSelectFolder = null;
		public Action<string> Config_OnSelectFile = null;
		...
		[Browsable(false)]
		internal ExplorerTreeView FolderView { get { return folderView; } }

		[Browsable(false)]
		internal ExplorerListView FileView { get { return fileView; } }

		[Browsable(false)]
		internal ExplorerComboBox NavAddressBox { get { return navAddressBox; } }
		...
		private void ChangeNavBarItem(SelectedFolderChangedEventArgs e) {
			...
			if (!navAddressBox.CurrentItem.ShellItem.Equals(e.Item)) {
				...
				ExplorerComboItem currentItem = navAddressBox.CurrentItem;
				...
				if (selectedNode.Nodes.Contains(e.Node) &&
					...
					navAddressBox.Items.Insert(currentIndex + 1, new ExplorerComboItem(e.Item, e.Node.Level));
					...
					while (currentItem.Text != e.Node.Text && currentItem.Indent >= maxIndent) {
						...
						currentIndex--;
						currentItem = (ExplorerComboItem)navAddressBox.Items[currentIndex];
					}
				} else {
					...
					if (folderView.IsParentNode(myCompNode, e.Node, out path)) {
						if (path.Length > 2) {
							...
							for (int i = 2; i < path.Length; i++) {
								navAddressBox.Items.Insert(startIndex + i,
									new ExplorerComboItem((ShellItem)path[i].Tag, i + 1));
							}
						...
						if (path.Length > 2) {
							...
							for (int i = 2; i < path.Length; i++) {
								navAddressBox.Items.Insert(startIndex + i,
									new ExplorerComboItem((ShellItem)path[i].Tag, i));
							}
		...
		void navAddressBox_SelectedIndexChanged(object sender, EventArgs e) {
			...
			if (navAddressBox.SelectedIndex > -1) {
				ShellItem item = ((ExplorerComboItem)navAddressBox.Items[navAddressBox.SelectedIndex]).ShellItem;
				...
				if (!selectedItem.Equals(item)) {
					...
					navAddressBox.CurrentItem = (ExplorerComboItem)navAddressBox.Items[navAddressBox.SelectedIndex];
